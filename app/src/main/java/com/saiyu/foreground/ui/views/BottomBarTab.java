...
public class BottomBarTab extends FrameLayout{
    ...
    private void init(Context context, int icon, CharSequence title) {
        ...
        LayoutParams tvUnReadParams = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
            ...
            mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.yellow));
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, R.color.yellow));
        } else {
    ...
    public void setTheme(int color) {
        mIcon.setColorFilter(ContextCompat.getColor(mContext, color));
        mTvTitle.setTextColor(ContextCompat.getColor(mContext, color));
    }
    ...
    public void setUnreadCount(int num) {
        if (num == -1) {
            mTvUnreadCount.setWidth(dip2px(mContext, 14));
            mTvUnreadCount.setHeight(dip2px(mContext, 14));
            mTvUnreadCount.setText(String.valueOf(""));
//            mTvUnreadCount.setWidth();
