...
public class CashFragment extends BaseFragment implements CallbackUtils.OnPositionListener,CallbackUtils.ResponseCallback,CallbackUtils.OnActivityCallBack,CashAdapter.OnItemClickListener,CashAdapter.OnSwipeListener,CashAdapter.OnItemLongClickListener {
    ...
    public void setOnResponseCallback(String method, BaseRet baseRet) {
        ...
        if(method.equals("CashFragment_cash")){
            ...
            DialogUtils.showCashChannelReviseDialog(getActivity(), reviseItem, ret.getData().getWithdrawWaysList(), new DialogUtils.DialogClickCallbackListener() {
                @Override
                public void setOnDialogClickCallbackListener(String wayId, String account, String remarks, String accountId) {
                    if(RiskLevel > 3){
                        ...
                        ApiRequest.addCashAccount(wayId,account,"",remarks,accountId,"CashFragment_addCashAccount",pb_loading);
                    } else {
                        //风控等级为1 2 3级，修改/添加渠道需要验证码
                        Bundle bundle = new Bundle();
                        bundle.putString("Mobile",Mobile);
                        bundle.putString("Id",wayId);
