...
public class ReceivePointAdapter extends RecyclerView.Adapter<ReceivePointAdapter.MyHolder> {
    List<ReceivePointRet.DatasBean.ItemsBean> mItems;
    private OnItemClickListener mOnItemClickListener;
    public void setOnItemClickListener(OnItemClickListener onItemClickListener) {
        this.mOnItemClickListener = onItemClickListener;
    }

    public ReceivePointAdapter(List<ReceivePointRet.DatasBean.ItemsBean> list){
        this.mItems = list;
    }

    @NonNull
    @Override
    public ReceivePointAdapter.MyHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_base_record, viewGroup, false);
        return new ReceivePointAdapter.MyHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ReceivePointAdapter.MyHolder myHolder, final int i) {
        if(mItems == null || mItems.size() <= i || mItems.get(i) == null){
            return;
        }

        myHolder.tv_deal_1.setText(mItems.get(i).getBizNote());
        int type = mItems.get(i).getType();
        String money = "";
        switch (type){
            case 0://收入+
                money = "<font color = \"#148cf1\">" + "+" + mItems.get(i).getPoint() + "</font>";
                break;
            case 1://支出-
                money = "<font color = \"#fe8f62\">" + "-" + mItems.get(i).getPoint() + "</font>";
                break;
        }
        myHolder.tv_deal_2.setText(Html.fromHtml(money));
        myHolder.tv_deal_3.setText(mItems.get(i).getCreateTime());
        myHolder.tv_deal_4.setText(mItems.get(i).getCurrentPoint());
    ...
    public void refreshData(List<ReceivePointRet.DatasBean.ItemsBean> list) {
        if (list != null) {
            mItems.clear();
            mItems.addAll(list);
