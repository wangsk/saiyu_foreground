...
public class RechargeStreamAdapter extends RecyclerView.Adapter<RechargeStreamAdapter.MyHolder> {
    List<RechargeStreamRet.DatasBean.ItemsBean> mItems;
    private OnItemClickListener mOnItemClickListener;
    public void setOnItemClickListener(OnItemClickListener onItemClickListener) {
        this.mOnItemClickListener = onItemClickListener;
    }

    public RechargeStreamAdapter(List<RechargeStreamRet.DatasBean.ItemsBean> list){
        this.mItems = list;
    }

    @NonNull
    @Override
    public RechargeStreamAdapter.MyHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.item_base_record, viewGroup, false);
        return new RechargeStreamAdapter.MyHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull RechargeStreamAdapter.MyHolder myHolder, final int i) {
        if(mItems == null || mItems.size() <= i || mItems.get(i) == null){
            return;
        }

        myHolder.tv_deal_1.setText(mItems.get(i).getReceiveOrderNum());
        myHolder.tv_deal_2.setText(mItems.get(i).getReceiveOrderStatus());
    ...
    public void refreshData(List<RechargeStreamRet.DatasBean.ItemsBean> list) {
        if (list != null) {
            mItems.clear();
            mItems.addAll(list);
